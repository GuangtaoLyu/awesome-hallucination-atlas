...
>
> 分类标注基于 **arXiv 论文摘要全文**自动分析（覆盖 1496/2063 篇），非仅标题关键词。
...
## 📊 数据概览
- **论文总数**：`2063` （已去重）
- **含论文链接**：`2060` · **含全文摘要**：`1496` · **含代码链接**：`1117` · **顶会正式发表**：`1007`
...
| 模型类型 | 说明 | 数量 |
|----------|------|------|
| **VLM** | 视觉语言模型（LVLM，含自称 MLLM 但仅处理图像/视频+文本的工作） | 786 |
...
| 方法类型 | 说明 | 数量 |
|----------|------|------|
| **Training-free** | 免训练（解码干预 / 注意力校准 / 表征引导等） | 1799 |
| **Training-based** | 基于训练（偏好优化 / 微调 / 强化学习等） | 264 |
...
<details>
<summary>「其他」明细 (159 venues, 233 papers — click to expand)</summary>
...
> 依据 **CCF 推荐国际学术会议 / 期刊目录（2026）** 对正式发表的论文标注评级；`未收录` 含 arXiv 预印本、暂未解析出 venue 的条目，以及 CCF 目录之外的会议 / 期刊。
...
## 📋 评测与 Benchmark
> 独立收录 `120` 篇评测 / Benchmark / 数据集论文（同时保留在下方主列表中，标有 📋）。

<details open>
<summary>📋 评测与 Benchmark 列表 (120 papers — click to collapse / expand)</summary>
